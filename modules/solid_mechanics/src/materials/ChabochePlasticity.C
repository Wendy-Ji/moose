#include "ChabochePlasticity.h"
#include "Function.h"
#include "ElasticityTensorTools.h"
#include "RadialReturnBackstressStressUpdateBase.h"
#include "ElasticityTensorTools.h"

registerMooseObject("SolidMechanicsApp", ADChabochePlasticity);
registerMooseObject("SolidMechanicsApp", ChabochePlasticity);

template <bool is_ad>
InputParameters
ChabochePlasticityTempl<is_ad>::validParams()
{
  ...
  params.addParam<FunctionName>("q_function", "Saturation value function for isotropic hardening (Q in Voce model)");
  params.addParam<FunctionName>("b_function", "Rate constant function for isotropic hardening (b in Voce model)");
  ...
  // params.addParam<Real>("C1", 0.0, "Kinematic hardening modulus (C1 in Chaboche model)");
  // params.addParam<Real>(
  //     "C2", 0.0, "Kinematic hardening modulus (C2 in Chaboche model)");
  // params.addParam<FunctionName>("gamma1", "The nonlinear hardening parameter (gamma) for back stress evolution");
  // params.addParam<FunctionName>("gamma2", "The nonlinear hardening parameter (gamma) for back stress evolution");
  params.addParam<FunctionName>("C1_function", "Kinematic hardening modulus function (C1 in Chaboche model)");
  params.addParam<FunctionName>("C2_function", "Kinematic hardening modulus function (C2 in Chaboche model)");
  params.addParam<FunctionName>("gamma1_function", "The nonlinear hardening parameter function (gamma) for back stress evolution");
  params.addParam<FunctionName>("gamma2_function", "The nonlinear hardening parameter function (gamma) for back stress evolution");

  return params;
}

template <bool is_ad>
ChabochePlasticityTempl<is_ad>::ChabochePlasticityTempl(
  ...
  : RadialReturnBackstressStressUpdateBaseTempl<is_ad>(parameters),
    ...
    // _C1(this->template getParam<Real>("C1")),
    // _C2(this->template getParam<Real>("C2")),
    // _gamma1(this->template getParam<Real>("gamma1")),
    // _gamma2(this->template getParam<Real>("gamma2")),
    // _q(this->template getParam<Real>("q")),
    // _b(this->template getParam<Real>("b")),
    _C1_function(this->isParamValid("C1_function")
                            ? &this->getFunction("C1_function")
                            : nullptr),
    _C2_function(this->isParamValid("C2_function")
                            ? &this->getFunction("C2_function")
                            : nullptr),
    _gamma1_function(this->isParamValid("gamma1_function") ? &this->getFunction("gamma1_function") : nullptr),
    _gamma2_function(this->isParamValid("gamma2_function") ? &this->getFunction("gamma2_function") : nullptr),
    _q_function(this->isParamValid("q_function") ? &this->getFunction("q_function") : nullptr),
    _b_function(this->isParamValid("b_function") ? &this->getFunction("b_function") : nullptr),
...
template <bool is_ad>
void
ChabochePlasticityTempl<is_ad>::computeStressInitialize(
...
template <bool is_ad>
GenericReal<is_ad>
ChabochePlasticityTempl<is_ad>::computeResidual(
...
template <bool is_ad>
void
ChabochePlasticityTempl<is_ad>::iterationFinalize(
...
template <bool is_ad>
void
ChabochePlasticityTempl<is_ad>::computeStressFinalize(
...
template <bool is_ad>
GenericReal<is_ad>
ChabochePlasticityTempl<is_ad>::computeIsotropicHardeningValue(
    const GenericReal<is_ad> & scalar)
{
...
template <bool is_ad>
void
ChabochePlasticityTempl<is_ad>::computeYieldStress(
    const GenericRankFourTensor<is_ad> & /*elasticity_tensor*/)
{
...
template class ChabochePlasticityTempl<false>;
template class ChabochePlasticityTempl<true>;
