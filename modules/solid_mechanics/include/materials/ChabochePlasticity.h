...
template <bool is_ad>
class ChabochePlasticityTempl
  : public RadialReturnBackstressStressUpdateBaseTempl<is_ad>
{
public:
  static InputParameters validParams();

  ChabochePlasticityTempl(const InputParameters & parameters);
...
protected:
  ...
  // Coefficients for chaboche hardening
  GenericReal<is_ad> _C1;
  GenericReal<is_ad> _C2;
  GenericReal<is_ad> _gamma1;
  GenericReal<is_ad> _gamma2;
  GenericReal<is_ad> _q;
  GenericReal<is_ad> _b;

  // temp dep functions for chaboche hardening
  const Function *  _C1_function;
  const Function * _C2_function;
  const Function * _gamma1_function;
  const Function * _gamma2_function;
  const Function * _q_function;
  const Function * _b_function;
  ///@}
...
typedef ChabochePlasticityTempl<false> ChabochePlasticity;
typedef ChabochePlasticityTempl<true> ADChabochePlasticity;
